...
1. **Fetch.** Download each `url` to a local cache keyed by `(id, version_pin)` via a temp file + atomic rename (a failed/partial download never leaves a usable-looking artefact). The download is skipped only when the cached artefact exists **and**, if a real `sha256` is pinned, matches it — a stale/corrupt cache for the same `(id, version_pin)` is re-fetched, not reused. Reproducible re-runs against a valid cache are network-free.
2. **Validate.** Each GTFS archive passes a built-in structural pre-flight (`validate_gtfs`): required files + a service calendar present, **referential integrity** (`trips` → routes/services, `stop_times` → trips/stops), and **strictly increasing `stop_sequence`** per trip. Ingestion is gated on it — any **ERROR**-level finding aborts (CLI exit 2); **WARNING**-level findings are logged but do not block. For full conformance on large national archives, the canonical MobilityData *gtfs-validator* (Coastline §1.1) may be run additionally. The OSM PBF is validated as non-empty with a valid PBF header (`OSMHeader`); deeper checks (way/node counts, corridor-`bbox` coverage) require a PBF parser and are **deferred** (a forward-hook behind `data/ingest.py`).
...
**GTFS validation tests.** `data/ingest.py` runs structural validation on every feed at ingest and the test suite asserts it: required files (`stops.txt`, `routes.txt`, `trips.txt`, `stop_times.txt`, `calendar*.txt`), referential integrity (every `stop_id`/`trip_id`/`route_id` referenced is defined, and every `trips.service_id` is defined in the calendar), and strictly increasing `stop_sequence` per trip. *Date-aware* service-calendar coverage of the planned departure date is **deferred** — it needs the departure threaded into ingest, and is a forward-hook on `validate_gtfs`. Validation failure must raise before graph build and map to CLI exit 2. A negative fixture (a deliberately corrupted feed in `src/rro/tests/data/broken/`) asserts the failure path.
